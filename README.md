...
one opaque string everywhere it is used: nothing splits it at the hyphen, nothing lowercases the
region, and every table — the dictionaries, the plural rules, the CSV separators — is keyed by the
whole of it.
...
an issue naming the key and a better wording is enough, and a pull request touching one language's
block is enough on its own.
...
[RFC 4180](https://datatracker.ietf.org/doc/html/rfc4180) asks for. In every other language of the
interface it is a **semicolon** — a deliberate departure from the standard, and for a concrete
reason: when Excel opens a `.csv` on a double click, it does not read the file by the RFC, it splits
it by the **list separator of the system locale**. The rule is not about any one language, it is
about the decimal mark: every locale that writes it as a comma sets the list separator to a
semicolon, and Russian, German, French, Spanish and Brazilian Portuguese all do. A comma separated
file opens there as one single column, and the user goes off to fight with the import wizard —
which means the table the whole thing was for did not open.
